...
def open_frame_directly(driver: webdriver.Chrome, wait: WebDriverWait, base_url: str | None = None) -> bool:
    ...
    time.sleep(1.0)
    try:
        if "OpeningFrame.aspx" in (driver.current_url or ""):
            return True
    except Exception:
        pass
    try:
        popup_iframe = None
        # Fallbacks según variaciones del overlay/iframe (incluye caso detectado en análisis: StepsProcessAward/OpeningFrame.aspx).
        selectors = [
            "#RadWindowWrapper_PopupFicha iframe",
            "iframe[src*='StepsProcessAward/OpeningFrame.aspx']",
            "iframe[src*='OpeningFrame.aspx']",
        ]
        for sel in selectors:
            try:
                popup_iframe = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, sel)))
                if popup_iframe:
                    break
            except TimeoutException:
                continue
        if not popup_iframe:
            raise TimeoutException("No se encontró iframe de OpeningFrame.")
    except TimeoutException:
...
def _navegar_a_supply_summary_directo(driver: webdriver.Chrome, base_url: str | None = None) -> str:
    """
    Si la página actual es un frameset (OpeningFrame), intenta detectar un frame/iframe cuyo src
    apunta a SupplySummary.aspx y navega directo a esa URL.
    Devuelve la URL abierta (o '' si no se pudo).
    """
    ...
    try:
        if "SupplySummary.aspx" in current:
            return current
    except Exception:
        pass
    ...
    for fr in frames:
        try:
            candidate = (fr.get_attribute("src") or "").strip()
        except Exception:
            candidate = ""
        if "SupplySummary.aspx" in candidate:
            src = candidate
            break
    ...
    try:
        full = src if src.startswith("http") else urljoin(current or base_url or BASE, src)
        print(f"[SCRAPE_CUADRO] Navegando directo a SupplySummary: {full}")
        driver.get(full)
        return full
    except Exception:
        return ""
...
def switch_to_supply_summary(driver: webdriver.Chrome) -> bool:
    """
    En la página actual, busca frame/iframe con SupplySummary.aspx y cambia a él si existe.
    """
    driver.switch_to.default_content()
    frames = driver.find_elements(By.TAG_NAME, "frame") + driver.find_elements(By.TAG_NAME, "iframe")
...
def switch_to_cuerpo_frame(driver: webdriver.Chrome, wait: WebDriverWait) -> bool:
    ...
    try:
        ...
        frame_el = None
        # Fallback: buscar por id/name parcial (algunos despliegues cambian el atributo).
        try:
            frames = driver.find_elements(By.TAG_NAME, "frame") + driver.find_elements(By.TAG_NAME, "iframe")
            for fr in frames:
                nm = (fr.get_attribute("name") or fr.get_attribute("id") or "").strip().lower()
                if nm and "cuerpo" in nm:
                    frame_el = fr
                    break
        except Exception:
            frame_el = None
...
def _switch_to_frame_containing_css(driver: webdriver.Chrome, css_selector: str, max_depth: int = 6) -> bool:
    """
    Busca recursivamente en frame/iframe hasta encontrar al menos un elemento que matchee el selector.
    Deja al driver posicionado en el frame donde se encontró.
    """
    ...
    def _has() -> bool:
        try:
            return bool(driver.find_elements(By.CSS_SELECTOR, css_selector))
        except Exception:
            return False
        ...
        for idx, fr in enumerate(frames):
            try:
                key = (
                    depth,
                    idx,
                    (fr.get_attribute("name") or fr.get_attribute("id") or "").strip(),
                    (fr.get_attribute("src") or "").strip(),
                )
                if key in visited:
                    continue
                visited.add(key)
    ...
    try:
        # Partir desde el contexto actual.
        if _dfs(0):
            return True
    except Exception:
        pass
...
def _extract_url_from_onclick(onclick: str) -> str:
    ...
    m = re.search(r"openPopUp\(['\"]([^'\"]*ViewBidAttachment\.aspx[^'\"]*)", onclick or "")
    if not m:
        return ""
...
def _requests_session_from_driver(driver: webdriver.Chrome) -> requests.Session:
    ...
    # Importante: conservar dominio/path para cookies host-only como ASP.NET_SessionId (www.mercadopublico.cl)
    try:
...
def _download_attachment_popup(
    ...
    """
    # Ajustar headers para el popup actual (algunas rutas validan Origin/Referer)
    try:
        session.headers["Referer"] = url
        parsed = urlparse(url)
        if parsed.scheme and parsed.netloc:
            session.headers.setdefault("Origin", f"{parsed.scheme}://{parsed.netloc}")
    except Exception:
        pass
    ...
    while pending_pages:
        ...
        if not search_names:
            search_names = re.findall(
                r"__doPostBack\(\s*['\"](DWNL\$grdId\$ctl\d+\$[^'\"]+)['\"]",
                html,
                flags=re.IGNORECASE,
            )
        if not search_names:
            search_names = re.findall(r'name="(DWNL\$grdId\$ctl\d+\$[^\"\\s]+)"', html)
        ...
        if meta:
            ...
            for name in search_names:
                ...
                vistos.add(name)
                parts = name.split("$")
                ctl = parts[-2] if len(parts) >= 2 else ""
                if ctl and ctl in meta:
                    filtrados.append(name)
            search_names = filtrados

        page_links = re.findall(
            r"__doPostBack\(\s*['\"]DWNL\$grdId['\"]\s*,\s*['\"]Page\$(\d+)['\"]\s*\)",
            html,
            flags=re.IGNORECASE,
        )
        for p in page_links:
...
def _save_stream_to_file(
    ...
    try:
        ...
        is_html = "text/html" in content_type and not dispo
        iterator = resp.iter_content(chunk_size=8192)
        first = next(iterator, b"")
        if is_html and first.lstrip().lower().startswith((b"<!doctype html", b"<html")):
            print(f"[WARN] Respuesta HTML en vez de archivo para {safe_name} (posible captcha/bloqueo).")
            return ""
        if not first:
            return ""
        with open(path, "wb") as f:
...
def _parse_state(html: str) -> dict:
    ...
    # Capturar todos los hidden (no solo VIEWSTATE), porque algunos flujos requieren campos extra.
    pairs = re.findall(
    ...
    # Algunos hidden pueden venir sin atributo value; asegurar clave con string vacío.
    names = re.findall(
...
def descargar_adjuntos_desde_url(url: str, driver: webdriver.Chrome, codigo: str | None = None, download_dir: str | None = None) -> dict:
    ...
    try:
        if not open_frame_directly(driver, wait, base_url=url):
            ...
            return resultado
        # En algunos framesets `readyState` no llega a complete; no bloquear el flujo por esto.
        try:
    ...
    try:
        print(f"[SCRAPE_CUADRO] URL tras abrir cuadro: {driver.current_url}")
    except Exception:
        pass

    # Preferir ir directo a SupplySummary (donde está #grdSupplies) para evitar complejidad de frameset.
    try:
        _navegar_a_supply_summary_directo(driver, base_url=url)
    except Exception:
        pass

    # Ubicar la tabla (puede estar en frameset <frame name='Cuerpo'> y/o en frame/iframe interno)
    driver.switch_to.default_content()
    found = False
    try:
        found = _switch_to_frame_containing_css(driver, "#grdSupplies", max_depth=8)
    except Exception:
        found = False
    ...
    if not found:
        resultado["errores"].append("No se encontró la tabla de ofertas (#grdSupplies) en ningún frame.")
        return resultado
    ...
    try:
        ...
        resultado["errores"].append(f"No se pudo confirmar la tabla de ofertas: {exc}")
        return resultado
